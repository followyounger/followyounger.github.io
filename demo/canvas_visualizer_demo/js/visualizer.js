...
AudioVisualizer.prototype.initialize = function () {
    ...
    this.cxt = this.wrap.getContext("2d");

    var WIDTH = window.innerWidth,
        HEIGHT = window.innerHeight;

    this.ratio = getRatio(this.ctx);
    ...
    this.wrap.width = WIDTH * this.ratio;
    this.wrap.height = HEIGHT * this.ratio;
    ...
    window.addEventListener('resize', function () {
        var WIDTH = window.innerWidth,
            HEIGHT = window.innerHeight;
        that.height = HEIGHT * that.ratio;
        that.width = WIDTH * that.ratio;
        that.wrap.style.height = HEIGHT + "px";
        that.wrap.style.width = WIDTH + "px";
        that.wrap.width = WIDTH * that.ratio;
        that.wrap.height = HEIGHT * that.ratio;
    });
...
function drawCurve(ctx, ptsa, tension, isClosed, numOfSegments) {
    ctx.beginPath();
...
AudioVisualizer.prototype.setupAudioProcessing = function () {
    ...
    let rotation = 0,
        barWidth = that.wrap.width / 180,
        rotationC = 0;

    const cxtWidth = 1,
        angle = 1;
    (function drawSpectrum() {
        visualizer.analyser.getByteFrequencyData(output);
        let potInt = { x: that.width / 2, y: that.height / 2 };
        let r = getmin(getmin(potInt.x - 50, potInt.y - 50), 175 * that.ratio);
        that.cxt.clearRect(0, 0, that.wrap.width, that.wrap.height);
        that.cxt.strokeStyle = "#C0F1FA";
        that.cxt.lineWidth = cxtWidth * that.ratio;
        that.cxt.fillStyle = "#C0F1FA";
        that.curvePoints.length = 0;
        that.circlePoints.length = 0;
        for (let i = 0; i < 180; i++) {
            let value = output[i] / 6 * that.ratio;
            that.cxt.beginPath();
            let Rv1 = (r - value);
            let Rv2 = (r + value);
            let temp = rotation * 2 + 540;
            that.cxt.moveTo(( Math.sin((i * angle + rotation) / 180 * Math.PI) * Rv1 + potInt.x),
                -Math.cos((i * angle + rotation) / 180 * Math.PI) * Rv1 + potInt.y);
            that.cxt.lineTo(( Math.sin((i * angle + rotation) / 180 * Math.PI) * Rv2 + potInt.x),
                -Math.cos((i * angle + rotation) / 180 * Math.PI) * Rv2 + potInt.y);
            that.cxt.moveTo(( Math.sin((temp - (i * angle + 180 + rotation)) / 180 * Math.PI) * Rv1 + potInt.x),
                -Math.cos((temp - (i * angle + 180 + rotation)) / 180 * Math.PI) * Rv1 + potInt.y);
            that.cxt.lineTo(( Math.sin((temp - (i * angle + 180 + rotation)) / 180 * Math.PI) * Rv2 + potInt.x),
                -Math.cos((temp - (i * angle + 180 + rotation)) / 180 * Math.PI) * Rv2 + potInt.y);
            that.cxt.stroke();
            barWidth = that.wrap.width / 180;
            that.cxt.fillRect(barWidth * i, that.wrap.height, barWidth - 1, -1 * value);
            if ( i % 3 === 0 || i === 179) {
                that.curvePoints.push(barWidth * (i + 0.5) - 0.5);
                that.curvePoints.push(that.wrap.height - value - 40);
            }
        }

        // 环状波形线
        that.circlePoints.push(Math.sin((rotationC) / 180 * Math.PI) * (r + output[0] / 4) + potInt.x);
        that.circlePoints.push(-Math.cos((rotationC) / 180 * Math.PI) * (r + output[0] / 4) + potInt.y);
        for (let i = 1; i < 90; ++i) {
            if (i % 3 === 0) {
                let value = output[i] / 6 * that.ratio;
                that.circlePoints.push(Math.sin((i * 4 + rotationC) / 180 * Math.PI) * (r - 10 * that.ratio + value) + potInt.x);
                that.circlePoints.push(-Math.cos((i * 4 + rotationC) / 180 * Math.PI) * (r - 10 * that.ratio + value) + potInt.y);
            }
        }

        if (that.played) {
            drawCurve(that.cxt, that.circlePoints, 0.5, true);
            drawCurve(that.cxt, that.curvePoints);
        }

        rotation += 0.1;
        rotationC -= 0.1;
        for (let i = 0; that.played && i < that.particles.length; i++) {
            if(that.particles[i].active === true) {
                that.particles[i].active = true;
                that.particles[i].x += that.particles[i].vx;
                that.particles[i].y += that.particles[i].vy;
                that.particles[i].hue -= 0.5;
                that.particles[i].r = Math.abs(that.particles[i].r - .05);
                that.cxt.beginPath();
                that.cxt.rect(that.particles[i].x, that.particles[i].y, that.particles[i].r, that.particles[i].r);
                that.cxt.fillStyle = "hsla(" + that.particles[i].hue + ",100%,50%,1)";
                that.cxt.fill();
                // reset particle
                if(that.particles[i].r <= .05) {
                    that.particles[i].active = false;
                }
            } else {
                that.particles[i].x = that.width / 2;
                that.particles[i].y = that.height / 2;
                that.particles[i].r = rand(12, 6, 1);
                that.particles[i].vx = Math.random() * 5 - 2.5;
                that.particles[i].vy = Math.sqrt((Math.random() * 6.5) - (that.particles[i].vx) * (that.particles[i].vx)) * (Math.random() < 0.5 ? -1 : 1);
                that.particles[i].opacity = Math.random() + 0.5;
                that.particles[i].active = true;
                that.cxt.beginPath();
                that.cxt.rect(that.particles[i].x, that.particles[i].y, that.particles[i].r, that.particles[i].r);
                that.cxt.fillStyle = "hsla(" + that.particles[i].hue + ",100%,50%,0.5)";
                that.cxt.fill();
            }
        }
...
AudioVisualizer.prototype.getAudio = function () {
    ...
    request.onload = function () {
        if (!that.played){
            jsmediatags.read(request.response, {
                onSuccess: function(tag) {
                    ...
                    document.querySelector('#audio-pic').src = "data:" + tag.tags.picture.format + ";base64," + window.btoa(base64String);
                },
                onError: function(error) {
                    console.log(error);
                    if (!document.querySelector('#audio-pic').classList.contains('hidden')) {
                        document.querySelector('#audio-pic').classList.add('hidden');
                    }
                }
            });
    ...
    function decodeAudioDataSuccess(decodedBuffer) {
        ...
        if (that.isauthorized){
            ...
            document.getElementById("play-icon").onclick = function(){
                ...
                $('#authorize')[0].remove();
                if (document.querySelector('#audio-pic').classList.contains('hidden')) {
                    document.querySelector('#audio-pic').classList.remove('hidden');
                }
            };
        }
    }
